...
import { E621Post } from '../api/e621/types';
import {
      ...
      return {
        ...
        downloads: state.downloads.map(download =>
          download.id === action.payload && download.status === 'paused'
            ? { ...download, status: 'downloading', startedAt: new Date() }
            : download
        ),
...
export function DownloadManagerProvider({ children }: { children: React.ReactNode }) {
  ...
  const value: DownloadManagerContextValue = {
    ...
    selectPost: (postId) => dispatch({ type: 'SELECT_POST', payload: postId }),
    deselectPost: (postId) => dispatch({ type: 'DESELECT_POST', payload: postId }),
    selectAllPosts: (postIds) => dispatch({ type: 'SELECT_ALL', payload: postIds }),
    deselectAllPosts: () => dispatch({ type: 'DESELECT_ALL' }),
    invertSelection: (allPostIds) => dispatch({ type: 'INVERT_SELECTION', payload: allPostIds }),
    downloadPost: (post, quality) => {
      ...
      if (post.file) {
        if (downloadQuality === 'original') {
          ...
          url = post.sample.url || '';
          // Since sample doesn't have a size property, estimate based on dimensions
          size = post.sample.width && post.sample.height 
            ? Math.round((post.sample.width * post.sample.height * 3) / 8) // Rough estimate
            : 0;
        } else if (post.preview) {
          url = post.preview.url || '';
          size = 0; // Preview size usually isn't provided in the API
        }
      }
    ...
    resumeDownload: (id) => dispatch({ type: 'RESUME_DOWNLOAD', payload: id }),
    removeDownload: (id) => {
    ...
    resumeAllDownloads: () => dispatch({ type: 'RESUME_ALL' }),
    clearCompletedDownloads: () => dispatch({ type: 'CLEAR_COMPLETED' }),
    clearQueue: () => dispatch({ type: 'CLEAR_QUEUE' }),
    updateSettings: (settings) => dispatch({ type: 'UPDATE_SETTINGS', payload: settings }),
    toggleDownloadPanel: () => dispatch({ type: 'TOGGLE_DOWNLOAD_PANEL' }),
    isPostSelected: (postId) => state.selection.selectedPosts.includes(postId),
  };
